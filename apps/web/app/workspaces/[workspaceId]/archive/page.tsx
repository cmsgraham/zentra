...
export default function ArchivePage() {
  ...
  const loadTasks = useCallback(async () => {
    const data = await api<{ items: TaskData[] }>(`/workspaces/${workspaceId}/tasks?archived=true&limit=200`);
    setTasks(data.items);
  }, [workspaceId]);
    ...
    <AuthShell>
      <div className="flex flex-1 h-[calc(100vh-57px)]">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {tasks.length === 0 ? (
            <p className="text-center py-16" style={{ color: 'var(--ink-text-muted)' }}>No archived tasks.</p>
          ) : (
